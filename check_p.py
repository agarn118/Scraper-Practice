...
def run_round(
    ...
    for query in queries:
        ...
        while True:
            ...
            for link in links:
                # Skip URLs we've already successfully scraped OR permanently given up on
                ...
                try:
                    ...
                    # but we DO track them and possibly retry later.
                    ...
                    if current_count >= MAX_PRODUCT_RETRIES:
                        ...
                        # We still have retry budget → mark query to retry in another round
...
def main():
    # Your output file: will always be appended to
    OUTPUT_FILE = "check_p.jsonl"

    seen_urls = set()              # URLs we've successfully scraped
    product_retry_counts = {}      # URL -> retry count
    failed_urls_final = set()      # URLs we've permanently given up on
    ...
    # OPEN IN APPEND MODE so we always add, never overwrite
    with open(OUTPUT_FILE, "a", encoding="utf-8") as file:
        ...
        while current_queries:
            ...
            if not failed_this_round:
                # No failed queries this round → we're done
                remaining_failed = []
                break

            failed_set = sorted(set(failed_this_round))

            # If the set of failing queries does not shrink, we stop retrying
